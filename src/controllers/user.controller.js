...
const generatorAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(500, "Access and refresh token failed to generate");
  }
};

const registerUser = asyncHandler(async (req, res) => {
  /// get user details from frontent e.g (id, username, password,)
  ...
  /// if req is coming from body/ form --> req.body
  ...
  if (!createdUser) {
    throw new ApiError(
      500,
      "Internal Error, something went wrong while registering the user"
    );
  }
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  /// find the user
  /// if user crediential do not match existing data then ask for signin user
  /// check if user already exist then login
  /// password check
  ...
  if (!isPasswordValid) {
    throw new ApiError(401, "Send a valid password");
  }
...
const refreshAccessToken = asyncHandler(async(req,res) => {
  ...
  try {
    ...
    throw new ApiError(401, error?.message ||"Internal server error during refreshAccessToken")
...
const changeCurrentPassword = asyncHandler(async(req,res) => {
  ...
  console.log(`Updated password value: ${user.password}`);
  await user.save(
    {
...
const updateAccountDetails = asyncHandler(async(req,res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    // console.log(`Update req.user details: ${req.user}`);
    // console.log(`Update req.user details: ${user}`);
  ...
  const channel = await User.aggregate(
    [
      ...
      },
      {
        $addFields: {
          subscribersCount:{
            $size: "$subscibers" //? Number of subscriber
          },
          channelsSubcribedToCount: {
            $size: "$subscribedTo" //? No of Channel
          },
          isSubscribed:{
            $cond:{
              if: {$in : [req.user?._id, "$subscibers.subcriber"]},
              then: true,
              else: false,
            }
          },
